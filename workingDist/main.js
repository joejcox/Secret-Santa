...
var names = []; // Add items to list when enter is pressed
...
roll.addEventListener('click', function () {
  ...
  if (person !== "Choose your name") {
    ...
    if (names.length === 1) {
      result.innerHTML = names[0];
      names = [];
    } else {
      do {
        getRandom = getRandomName();
      } while (getRandom === person);

      result.innerHTML = getRandom;
    }

    var index = names.indexOf(getRandom);
...
function updateList() {
  ...
  for (var i = 0; i < names.length; i++) {
    html += '<div class="row"><div class="name">';
    html += names[i];
    html += '</div><div class="delete"><button class="danger remove">Delete</button></div></div>';
    list.innerHTML = html;
  }
